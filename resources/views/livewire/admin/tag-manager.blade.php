<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl font-bold text-gray-800">Manajemen Tag</h1>
        @can('tags-create')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-4 flex flex-col sm:flex-row gap-3">
        <div class="flex-1">
            <input wire:model.live="search" type="text" placeholder="Cari nama tag..."
                   class="w-full sm:w-80 border-gray-300 focus:border-[#1A6FAA] focus:ring-[#1A6FAA] rounded-lg text-sm">
        </div>
        <div class="sm:w-32">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 flex items-center justify-between" x-data="{ showDeleteModal: false }">
            ...
            <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
                ...
                <div class="relative bg-white rounded-xl shadow-lg p-6 max-w-sm w-full mx-4">
                    ...
                    <p class="text-sm text-gray-600 mb-4">Yakin ingin menghapus {{ count($selected) }} tag yang dipilih? Tindakan ini tidak dapat dibatalkan.</p>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                <thead class="bg-gray-50 text-gray-600 font-medium border-b border-gray-200">
                    <tr>
                        <th class="px-5 py-3 w-10">
                            ...
                            @endphp
                            <input type="checkbox" wire:click.prevent="toggleSelectPage" @checked($allPageSelected) wire:key="select-all-page-{{ $tags->currentPage() }}" class="rounded border-gray-300 text-[#1A6FAA] focus:ring-[#1A6FAA]">
                        </th>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($tags as $tag)
                        <tr class="hover:bg-gray-50 transition-colors" wire:key="row-{{ $tag->id }}">
                            <td class="px-5 py-3.5">
                                <input type="checkbox" wire:model.live="selected" value="{{ $tag->id }}" class="rounded border-gray-300 text-[#1A6FAA] focus:ring-[#1A6FAA]">
                            </td>
                            <td class="px-5 py-3.5 font-medium text-gray-800">{{ $tag->name }}</td>
                            <td class="px-5 py-3.5 text-gray-500">{{ $tag->slug }}</td>
                            ...
                            <td class="px-5 py-3.5 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    @can('tags-edit')
                                    <button wire:click="edit({{ $tag->id }})" class="p-1.5 text-gray-500 hover:text-[#1A6FAA] hover:bg-blue-50 rounded-md transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                    </button>
                                    @endcan
                                    @can('tags-delete')
                                    <button wire:click="confirmDelete({{ $tag->id }})" class="p-1.5 text-gray-500 hover:text-red-600 hover:bg-red-50 rounded-md transition-colors" title="Hapus">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-5 py-8 text-center text-gray-500">Tidak ada data tag.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <x-modal name="tag-modal" :show="false">
        <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ $editingId ? 'Edit Tag' : 'Tambah Tag' }}</h3>
    ...
    @if($confirmingDelete)
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 z-10 p-6">
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Konfirmasi Hapus</h3>
                <p class="text-sm text-gray-500 mb-6">Apakah Anda yakin ingin menghapus tag ini? Data akan dipindahkan ke tong sampah.</p>
