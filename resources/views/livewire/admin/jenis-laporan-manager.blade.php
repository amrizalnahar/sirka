<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-xl font-bold text-gray-800">Master Data Jenis Laporan</h1>
        @can('jenis-laporan-create')
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4 flex items-center justify-between" x-data="{ showDeleteModal: false }">
            ...
            <div x-show="showDeleteModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center" style="display: none;">
                ...
                <div class="relative bg-white rounded-xl shadow-lg p-6 max-w-sm w-full mx-4">
                    ...
                    <p class="text-sm text-gray-600 mb-4">Yakin ingin menghapus {{ count($selected) }} jenis laporan yang dipilih?</p>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse ($items as $item)
                        <tr class="hover:bg-gray-50 transition-colors" wire:key="row-{{ $item->id }}">
                            ...
                            <td class="px-5 py-3.5 text-gray-500">{{ $item->deskripsi ?: '-' }}</td>
                            <td class="px-5 py-3.5">
                                @if($item->is_active)
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-gray-500">Tidak ada data jenis laporan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <!-- Modal Form -->
    <x-modal name="jenis-laporan-modal" :show="false">
        <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-800 mb-4">{{ $editingId ? 'Edit Jenis Laporan' : 'Tambah Jenis Laporan' }}</h3>

            <form wire:submit="save">
                <div class="space-y-4">
                    <div>
                        <x-input-label for="kode" value="Kode" />
                        <x-text-input id="kode" wire:model="kode" type="text" class="mt-1 block w-full" placeholder="Contoh: LRA" />
                        <x-input-error :messages="$errors->get('kode')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="nama" value="Nama" />
                        <x-text-input id="nama" wire:model="nama" type="text" class="mt-1 block w-full" placeholder="Masukkan nama jenis laporan" />
                        <x-input-error :messages="$errors->get('nama')" class="mt-1" />
                    </div>

                    <div>
                        <x-input-label for="deskripsi" value="Deskripsi" />
                        <textarea id="deskripsi" wire:model="deskripsi" rows="3" class="mt-1 block w-full border-gray-300 focus:border-[#1A6FAA] focus:ring-[#1A6FAA] rounded-md shadow-sm text-sm" placeholder="Deskripsi singkat (opsional)"></textarea>
                ...
                <div class="mt-6 flex justify-end gap-3">
                    ...
                    <x-primary-button type="submit">{{ $editingId ? 'Simpan Perubahan' : 'Tambah Jenis Laporan' }}</x-primary-button>
                </div>
            </form>
        </div>
    </x-modal>
    ...
    @if($confirmingDelete)
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="bg-white rounded-xl shadow-xl max-w-md w-full mx-4 z-10 p-6">
                <h3 class="text-lg font-semibold text-gray-800 mb-2">Konfirmasi Hapus</h3>
                <p class="text-sm text-gray-500 mb-6">Apakah Anda yakin ingin menghapus jenis laporan ini? Data akan dipindahkan ke tong sampah.</p>
